...
export async function generateModelRollout(
  ...
  const sys = 'You are a strategic growth planner. Return ONLY valid JSON. Be specific and KPI-driven.'
  const user = [
    `Create a 12-month rollout strategy for ${persona || 'a creator'}${region?` in ${region}`:''}.`,
    targetAudience ? `Target audience: ${targetAudience}.` : '',
    context ? `\nKPI Context:\n${context}` : '',
    `\nConcept Overview:\n${overview}`,
    `\nGenerate strategic rollout JSON with:`,
    ``,
    `1. months: Array of 12 objects with { m: 0-11, followers: number }`,
    `   - Realistic growth reflecting campaign momentum`,
    `   - Spikes align with moments below`,
    ``,
    `2. moments: Array of 5-7 objects { m, label, desc, kind, color }`,
    `   - "label": Milestone name (e.g. "Album Drop", "Collab Push")`,
    `   - "desc": 1-2 sentences explaining WHAT happens and HOW it boosts KPIs (narrative/commercial/cross-platform)`,
    `   - "kind": launch|strategy|collab|viral|risk|pivot|milestone`,
    `   - Strategically timed to maximize KPI impact`,
    ``,
    `3. phases: Array of 3-5 objects { startM, endM, label, summary, kind, color }`,
    `   - Create phases that FIT this concept (not generic "Tease/Launch/Sustain")`,
    `   - "label": Phase name reflecting actual strategy (e.g. "Community Building", "Viral Push")`,
    `   - "summary": Brief explanation of phase focus and KPI targets`,
    ``,
    `4. notes: Array of 2-4 strings`,
    `   - Concept-specific risks`,
    `   - KPI checkpoints (when to pivot)`,
    `   - Contingencies if early moments underperform`,
  ].filter(Boolean).join('\n')
