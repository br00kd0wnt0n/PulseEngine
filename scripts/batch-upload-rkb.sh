...
find "$UPLOAD_DIR" -maxdepth 1 -type f ! -name ".*" ! -name "README.md" | while read -r filepath; do
    ...
    if [ "$http_code" -eq 201 ]; then
        ...
        asset_id=$(echo "$body" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data[0]['id'] if data and len(data) > 0 else '')" 2>/dev/null || echo "unknown")

        echo -e "${GREEN}✓ Uploaded successfully${NC} (ID: $asset_id)"
